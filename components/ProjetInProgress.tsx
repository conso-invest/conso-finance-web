        ...
        <div>
            <div className="flex justify-between items-center px-4 py-10 lg:px-20">
                <div className="w-full">
                    <h1 className="text-3xl lg:text-3xl font-extrabold"><span className="bg-primarycolor px-4 py-2 rounded text-white">Les projets en cours</span></h1>
                    <div className="mt-5 overflow-auto lg:w-full whitespace-nowrap scroll-smooth no-scrollbar">
                        {categoryList.slice(0, 9).map((item: any) => (
                            <span key={item.id} className={`${activeItem === item.id ? 'border-b-2 border-primarycolor' : ''} cursor-pointer hover:border-b-2 border-primarycolor mr-4`} onClick={() => getProjetByCategoryID(item.id)} >{item.description}</span>
                        ))}
                    </div>
                </div>
